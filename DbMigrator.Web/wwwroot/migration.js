/* ============================================================================
   MIGRATION DASHBOARD LOGIC - migration.js
   ============================================================================ */
        ...
        return `
            <div class="table-item sortable-item${disabledClass}" draggable="${isFilterActive ? 'false' : 'true'}" data-sort-id="${mapId}">
                ...
                <div class="table-actions">
                    ...
                    <button class="btn-icon" onclick="generateSpScript(${mapId})" title="Generate Stored Procedure (SP)" style="color: var(--accent-teal);">
                        <i class="fa-solid fa-file-code"></i>
                    </button>
                    <button class="btn-icon" onclick="openColumnMappingModal(${mapId}, '${sourceName.replace(/'/g, "\\'")}', '${targetName.replace(/'/g, "\\'")}')" title="Petakan Kolom Dinamis">
                        <i class="fa-solid fa-sliders"></i>
                    </button>
                    <button class="btn-icon" onclick="editTableMapping(${mapId})" title="Edit Pemetaan Tabel">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </button>
                    <button class="btn-icon delete" onclick="deleteTableMapping(${mapId})" title="Hapus Pemetaan">
                        <i class="fa-solid fa-trash-can"></i>
                    </button>
                </div>
            </div>
    ...
    container.innerHTML = items.map((item, idx) => {
        ...
        return `
            <label class="scan-item" data-type="${item.ObjectType || item.objectType}">
                <input type="checkbox" class="scan-checkbox" data-name="${escapeHtml(objName)}" data-type="${item.ObjectType || item.objectType}">
                <span class="scan-item-name">${escapeHtml(objName)}</span>
                ...
                <div class="backup-card">
                    <div style="display: flex; align-items: center; gap: 1rem;">
                        ...
                        <div>
                            ...
                            <div style="font-size: 0.7rem; color: var(--text-dark); font-family: Consolas, monospace; margin-top: 0.25rem; max-width: 400px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${escapeHtml(scriptPreview)}...</div>
                        </div>
                    </div>
...
async function startCleanTableScan() {
    ...
    try {
        ...
        cleanTableScanResultsCache = await res.json();
    ...
    container.innerHTML = tables.map(name => {
        return `
            <label class="scan-item">
                <input type="checkbox" class="clean-scan-checkbox" data-name="${escapeHtml(name)}">
                <span class="scan-item-name">${escapeHtml(name)}</span>
